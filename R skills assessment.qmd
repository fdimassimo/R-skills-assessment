  ...
  result <- lapply(poverty_lines, function(line) {
    df %>%
      summarise(
        ...
        povseverity = sum(weight * ((1 - (income < line) / line)^2)) / sum(weight)
      ) %>%
...
final_table <- do.call(rbind, poverty_estimates)
final_table$year <- as.numeric(gsub("\\..*", "", rownames(final_table)))
rownames(final_table) <- NULL
final_table <- final_table[, c("year", "pov_line", "headcount", "povgap", "povseverity")]
#final_table$headcount <- sprintf("%.15f", final_table$headcount)
#final_table$povgap <- sprintf("%.15f", final_table$povgap)

# Graph
ggplot(final_table, aes(x = year, y = headcount, color = factor(pov_line))) +
  geom_line() +
  geom_point() +
